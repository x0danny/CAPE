...
st.caption(f"💱 ERPsim data was originally in EUR. Revenue values are converted to USD at €1 = ${EUR_TO_USD:.2f}.")
st.divider()
...
with col2:
    ...
    st.caption("Time periods where carbon emissions were unusually high. More high-risk periods = more avoidable emissions.")
with col3:
    ...
    st.caption(f"This period had the highest risk score ({worst_score:.3f}). Carbon emissions per dollar of revenue were nearly double the safe level.")
with col4:
...
with col_c:
    fig3 = px.scatter(period_summary, x='late_pct', y='co2e_per_dollar',
                      ...
                      title='Late Orders vs Carbon Emissions',
                      labels={'late_pct': '% of Orders Late', 'co2e_per_dollar': 'Carbon Emissions per $1 of Revenue'},
                      hover_data=['period'])
    st.plotly_chart(fig3, use_container_width=True)
    st.caption("📌 **What this means:** Each dot is a time period. Dots in the top-right corner had both high delays AND high carbon emissions — the worst combination. Bigger dots = more wasted inventory carbon. This proves late orders and carbon spikes go together.")
...
with col_d:
    ...
    st.caption("📌 **What this means:** Phase 3 (Peak Risk) had the highest average risk — late orders spiked, overstock piled up, and carbon emissions climbed across the board.")
...
for _, row in high_risk.sort_values('cape_risk_score', ascending=False).iterrows():
    ...
    with st.expander(f"{level} — Period {row['period']} | Risk Score: {risk:.3f}"):
        ...
        col1.metric("Carbon Emissions per $1", f"{row['co2e_per_dollar']:.4f} kg",
        ...
        if overstock_val > 8000:
            ...
            st.write(f"{rec_num}. Inventory waste was relatively contained at {overstock_val:,.0f} kg. The bigger problem here was the carbon impact of delivery patterns, not warehouse buildup.")
        rec_num += 1
        ...
        if late_pct >= 80:
            ...
            st.write(f"{rec_num}. No new late orders in {period} — the high carbon emissions here came from overstock that accumulated in earlier periods.")
        rec_num += 1

        if intensity > 0.15:
            st.write(f"{rec_num}. Carbon intensity of {intensity:.4f} kg per dollar is {((intensity/0.09)-1)*100:.0f}% above the safe threshold. Switching even one shipment from air to ground would have saved significant emissions (air freight produces ~47-50x more carbon per ton-mile).")
        elif intensity > 0.12:
            st.write(f"{rec_num}. Carbon intensity of {intensity:.4f} kg per dollar is well above safe levels. Moving some shipments to ground transport would have meaningfully reduced the carbon footprint.")
        else:
            st.write(f"{rec_num}. Carbon intensity of {intensity:.4f} kg per dollar is moderately elevated. Combined with the inventory waste, this period's total carbon exposure exceeded acceptable levels.")
...
|------|-------------|---------------|
| **1. Order Placed** | A purchase order enters the system | Nothing has gone wrong yet — carbon emissions are zero |
